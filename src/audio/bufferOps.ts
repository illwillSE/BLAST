...
function makeBuffer(channels: Float32Array<ArrayBuffer>[], sampleRate: number): AudioBuffer {
...
function channelsOf(buffer: AudioBuffer): Float32Array<ArrayBuffer>[] {
...
export function normalizeBuffer(buffer: AudioBuffer): AudioBuffer {
  ...
  if (peak > 0.0001) {
    ...
    for (const d of channels) for (let i = 0; i < d.length; i++) d[i] = d[i]! * gain
  }
  return makeBuffer(channels, buffer.sampleRate)
}

// Linear fade over 15% of the buffer, capped at 0.5s.
export function fadeBuffer(buffer: AudioBuffer, direction: 'in' | 'out'): AudioBuffer {
  const channels = channelsOf(buffer)
  ...
  for (const d of channels) {
    for (let i = 0; i < fadeLen; i++) {
      ...
      if (direction === 'in') d[i] = d[i]! * gain
      else d[n - 1 - i] = d[n - 1 - i]! * gain
    }
  }
  return makeBuffer(channels, buffer.sampleRate)
}

export function cropBuffer(buffer: AudioBuffer, startSec: number, endSec: number): AudioBuffer | null {
