...
const smooth = (cur, target, amount) => cur + (target - cur) * amount
...
function buildTextTargets(width, height, word) {
...
function syncDots(dots, targets, nearest = false) {
  ...
  useEffect(() => {
    ...
    const draw = (now) => {
      ...
      for (let i = 0; i < dotsRef.current.length; i++) {
        ...
        const sampleX = Number.isFinite(audioValues[sampleIndexX]) ? audioValues[sampleIndexX] : 0
        const sampleY = Number.isFinite(audioValues[sampleIndexY]) ? audioValues[sampleIndexY] : 0
        const sampleZ = Number.isFinite(audioValues[sampleIndexZ]) ? audioValues[sampleIndexZ] : 0
