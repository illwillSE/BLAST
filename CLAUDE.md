...
**Auto-save:** on every project change, the project JSON is written to `localStorage['blast_autosave']` (1 s debounce, first render skipped). On init the `useUndoableProject` lazy initializer reads it back and runs `normalizeProject` (exported from `src/utils/projectZip.js`) so new-param backfill still applies. Sample blobs are not persisted in localStorage — they live in `sampleCache` and must be re-loaded after a page refresh (IndexedDB persistence is a planned follow-up).
